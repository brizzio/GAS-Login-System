...
function getUserByEmail(email) {
  ...
  for (var i = 1; i < data.length; i++) { // Skip header row
    if (data[i][1].toLowerCase() === email.toLowerCase()) { // Column B is Email (index 1)
...
function getUserByToken(token) {
  ...
  var data = sheet.getDataRange().getValues();
  for (var i = 1; i < data.length; i++) { // Skip header row
    if (data[i][8] && data[i][8] === token) { // Column I is Verification Token (index 8)
      return { row: i, data: data[i] };
    }
  }
  return null;
...
function processSignup(formData) {
  try {
    ...
    sheet.appendRow([
      formData.name,
      formData.email,
      hashedPassword, // Column C (index 2) - Storing the hashed password
      "Allowed",
      REDIRECT_URL,
      "", // OTP - Column F (index 5)
      "", // DATE OF EXPIRY - Column G (index 6)
      "Pending", // Status: Pending verification - Column H (index 7)
      verificationToken, // Verification Token - Column I (index 8)
      salt // Salt - Column J (index 9)
    ]);
...
function processLogin(formData) {
  try {
    ...
    var storedHashedPassword = userData[2]; // Column C is Hashed Password (index 2)
    var storedSalt = userData[9]; // Column J is Salt (index 9)
    var status = userData[7]; // Column H is Status (index 7)
    var type = userData[3]; // Column D is Type (index 3)
    ...
    if (hashedEnteredPassword == storedHashedPassword) {
      return { success: true, message: "Login successful!", redirectUrl: userData[4] || REDIRECT_URL }; // Column E is URL (index 4)
    } else {
      return { success: false, message: "Invalid email or password." };
    }
...
function resendVerificationLink(email) {
  try {
    ...
    updateUserField(user.row, 8, newVerificationToken); // Update Verification Token column I (index 8)
